...
import { getRowContent } from "../../lib/rows";
import { sectionOf } from "../sections/index";

export function registerGetRowContent(
	fastify: FastifyInstance,
	ctx: ContentCtx,
): void {
	fastify.get("/sections/:id/rows/:rowId/content", async (request, reply) => {
		const node = await sectionOf(request, reply, ctx.db);
		if (!node) return reply;
		const { rowId } = request.params as { rowId: string };
		const row = await getRowContent(ctx.db, node, rowId);
