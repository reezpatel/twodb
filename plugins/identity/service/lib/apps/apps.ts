...
export function appRoleClaims(
	...
	roleDefaults: {
		manager?: readonly Claim[];
		editor?: readonly Claim[];
		reader?: readonly Claim[];
	},
...
export async function effectiveAppClaims(
	...
	const ownerGrant = await isAppOwnerImplicit(db, userId, appWorkspaceId);
	if (ownerGrant) {
...
async function isAppOwnerImplicit(
	...
	const rows = await db
		...
		.innerJoin("roles", "roles.id", "workspace_role_assignments.role_id")
		.select("roles.key")
