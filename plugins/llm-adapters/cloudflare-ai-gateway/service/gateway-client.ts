...
type GatewayMessage =
  ...
  | { role: "user"; content: string | GatewayContentPart[] }
...
export function buildGatewayUrl(accountId: string, gatewayId: string): string {
  const account = encodeURIComponent(accountId.trim());
  const gateway = encodeURIComponent(gatewayId.trim());
...
function toContentParts(parts: TwodbContentPart[]): GatewayContentPart[] {
...
function toMessages(request: TwodbCompletionRequest): GatewayMessage[] {
  return request.messages.map((message): GatewayMessage => {
...
function buildBody(request: TwodbCompletionRequest, stream: boolean): GatewayRequestBody {
  return {
    model: request.model,
...
function mapUsage(usage: GatewayUsage | null | undefined): TwodbUsage | undefined {
...
async function post(url: string, body: GatewayRequestBody, options: GatewayClientOptions): Promise<Response> {
...
export async function completeChatCompletion(request: TwodbCompletionRequest, options: GatewayClientOptions): Promise<TwodbCompletionResponse> {
  const response = await post(options.url, buildBody(request, false), options);
  const payload = parseJson<GatewayResponse>(await response.text());
  return {
    id: payload.id ?? "",
...
export async function* streamChatCompletion(request: TwodbCompletionRequest, options: GatewayClientOptions): AsyncGenerator<TwodbStreamEvent> {
  ...
  while (true) {
    ...
    while (boundary !== null) {
      ...
      for (const line of rawEvent.split("\n")) {
        ...
        const chunk = parseJson<GatewayStreamChunk>(data);
        if (!started) {
          started = true;
