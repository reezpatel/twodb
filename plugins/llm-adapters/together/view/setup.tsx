...
function mergeConfig(raw: unknown): TogetherConnectionConfig {
  const stored = (raw ?? {}) as Partial<TogetherConnectionConfig>;
  return {
    api_key: stored.api_key ?? "",
  };
}

export function TogetherSetup({ connectionId, config, setConfig }: LlmProviderSetupProps) {
  const [current, setCurrent] = useState<TogetherConnectionConfig>(() => mergeConfig(config));
  const [status, setStatus] = useState<"idle" | "saving" | "saved" | "error">("idle");

  const field = (key: keyof TogetherConnectionConfig, value: string) => {
