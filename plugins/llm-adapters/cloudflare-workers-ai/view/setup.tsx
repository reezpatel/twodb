import { useState } from "react";
import { Input } from "@twodb/ui";
import type { LlmProviderSetupProps } from "@twodb/shared-frontend";
import type { CloudflareWorkersAiConnectionConfig } from "../shared/api";

function mergeConfig(raw: unknown): CloudflareWorkersAiConnectionConfig {
  const stored = (raw ?? {}) as Partial<CloudflareWorkersAiConnectionConfig>;
  return {
    account_id: stored.account_id ?? "",
    api_token: stored.api_token ?? "",
  };
}

export function CloudflareWorkersAiSetup({ connectionId, config, setConfig }: LlmProviderSetupProps) {
  const [current, setCurrent] = useState<CloudflareWorkersAiConnectionConfig>(() => mergeConfig(config));
  const [status, setStatus] = useState<"idle" | "saving" | "saved" | "error">("idle");

  const field = (key: keyof CloudflareWorkersAiConnectionConfig, value: string) => {
    const next = { ...current, [key]: value };
    setCurrent(next);
    if (value === current[key]) return;
    setStatus("saving");
    void setConfig(next).then((result) => setStatus(result.success ? "saved" : "error"));
  };

  return (
    <div key={connectionId} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: "var(--text-sm)", color: "var(--ink-3)" }}>
        Account ID
        <Input
          type="text"
          defaultValue={current.account_id}
          placeholder="Cloudflare account id"
          onBlur={(event) => field("account_id", event.target.value.trim())}
        />
      </label>
      <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: "var(--text-sm)", color: "var(--ink-3)" }}>
        API token
        <Input
          type="password"
          defaultValue={current.api_token}
          placeholder="Cloudflare API token"
          onBlur={(event) => field("api_token", event.target.value.trim())}
        />
      </label>
      {status !== "idle" && status !== "saved" ? (
        <span style={{ fontSize: "var(--text-sm)", color: status === "error" ? "var(--danger)" : "var(--ink-3)" }}>
          {status === "saving" ? "Saving…" : "Save failed"}
        </span>
      ) : null}
    </div>
  );
}
