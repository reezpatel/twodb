...
function buildHeaders(apiKey: string, org: string): Record<string, string> {
...
function toContentParts(parts: TwodbContentPart[]): OpenAiContentPart[] {
...
function toMessages(request: TwodbCompletionRequest): OpenAiMessage[] {
  return request.messages.map((message): OpenAiMessage => {
...
function buildBody(request: TwodbCompletionRequest, stream: boolean): OpenAiRequestBody {
  return {
    model: request.model,
...
function mapUsage(usage: OpenAiUsage | null | undefined): TwodbUsage | undefined {
...
async function post(url: string, body: OpenAiRequestBody, options: OpenAiClientOptions): Promise<Response> {
  const response = await fetch(url, {
    method: "POST",
    headers: buildHeaders(options.apiKey ?? "", options.org ?? ""),
...
export async function completeChatCompletion(request: TwodbCompletionRequest, options: OpenAiClientOptions): Promise<TwodbCompletionResponse> {
...
export async function* streamChatCompletion(request: TwodbCompletionRequest, options: OpenAiClientOptions): AsyncGenerator<TwodbStreamEvent> {
  ...
  while (true) {
    ...
    while (boundary !== null) {
      ...
      for (const line of rawEvent.split("\n")) {
        ...
        const chunk = parseJson<OpenAiStreamChunk>(data);
        if (!started) {
          started = true;
