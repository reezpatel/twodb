...
function mergeConfig(raw: unknown): MinimaxConnectionConfig {
  const stored = (raw ?? {}) as Partial<MinimaxConnectionConfig>;
  return {
    api_key: stored.api_key ?? "",
  };
}

export function MinimaxSetup({ connectionId, config, setConfig }: LlmProviderSetupProps) {
  const [current, setCurrent] = useState<MinimaxConnectionConfig>(() => mergeConfig(config));
  const [status, setStatus] = useState<"idle" | "saving" | "saved" | "error">("idle");

  const field = (key: keyof MinimaxConnectionConfig, value: string) => {
    ...
    <div key={connectionId} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: "var(--text-sm)", color: "var(--ink-3)" }}>
        ...
        <Input
          ...
          placeholder="minimax.io → API keys"
