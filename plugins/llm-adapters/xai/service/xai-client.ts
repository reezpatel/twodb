...
type XaiContentPart = { type: "text"; text: string } | { type: "image_url"; image_url: { url: string; detail?: string } };
...
function toContentParts(parts: TwodbContentPart[]): XaiContentPart[] {
...
function toMessages(request: TwodbCompletionRequest): XaiMessage[] {
  return request.messages.map((message): XaiMessage => {
...
function buildBody(request: TwodbCompletionRequest, stream: boolean): XaiRequestBody {
...
function mapUsage(usage: XaiUsage | null | undefined): TwodbUsage | undefined {
...
async function post(url: string, body: XaiRequestBody, options: XaiClientOptions): Promise<Response> {
...
export async function completeChatCompletion(request: TwodbCompletionRequest, options: XaiClientOptions): Promise<TwodbCompletionResponse> {
  const response = await post(options.url, buildBody(request, false), options);
  const payload = parseJson<XaiResponse>(await response.text());
  return {
    id: payload.id ?? "",
...
export async function* streamChatCompletion(request: TwodbCompletionRequest, options: XaiClientOptions): AsyncGenerator<TwodbStreamEvent> {
  ...
  while (true) {
    ...
    while (boundary !== null) {
      ...
      for (const line of rawEvent.split("\n")) {
        ...
        const chunk = parseJson<XaiStreamChunk>(data);
        if (!started) {
          started = true;
