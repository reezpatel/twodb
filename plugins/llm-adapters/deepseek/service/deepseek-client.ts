...
type DeepSeekContentPart = { type: "text"; text: string } | { type: "image_url"; image_url: { url: string; detail?: string } };
...
function toContentParts(parts: TwodbContentPart[]): DeepSeekContentPart[] {
...
function toMessages(request: TwodbCompletionRequest): DeepSeekMessage[] {
  return request.messages.map((message): DeepSeekMessage => {
...
function buildBody(request: TwodbCompletionRequest, stream: boolean): DeepSeekRequestBody {
  return {
    model: request.model,
...
function mapUsage(usage: DeepSeekUsage | null | undefined): TwodbUsage | undefined {
...
async function post(url: string, body: DeepSeekRequestBody, options: DeepSeekClientOptions): Promise<Response> {
...
export async function completeChatCompletion(request: TwodbCompletionRequest, options: DeepSeekClientOptions): Promise<TwodbCompletionResponse> {
  const response = await post(options.url, buildBody(request, false), options);
  const payload = parseJson<DeepSeekResponse>(await response.text());
  return {
    id: payload.id ?? "",
...
export async function* streamChatCompletion(request: TwodbCompletionRequest, options: DeepSeekClientOptions): AsyncGenerator<TwodbStreamEvent> {
  ...
  while (true) {
    ...
    while (boundary !== null) {
      ...
      for (const line of rawEvent.split("\n")) {
        ...
        const chunk = parseJson<DeepSeekStreamChunk>(data);
        if (!started) {
          started = true;
