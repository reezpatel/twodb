...
function mergeConfig(raw: unknown): GroqConnectionConfig {
  const stored = (raw ?? {}) as Partial<GroqConnectionConfig>;
  return {
    api_key: stored.api_key ?? "",
  };
}

export function GroqSetup({ connectionId, config, setConfig }: LlmProviderSetupProps) {
  const [current, setCurrent] = useState<GroqConnectionConfig>(() => mergeConfig(config));
  const [status, setStatus] = useState<"idle" | "saving" | "saved" | "error">("idle");

  const field = (key: keyof GroqConnectionConfig, value: string) => {
  ...
  return (
    <div key={connectionId} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: "var(--text-sm)", color: "var(--ink-3)" }}>
        API key
        <Input type="password" defaultValue={current.api_key} placeholder="gsk_…" onBlur={(event) => field("api_key", event.target.value.trim())} />
      </label>
