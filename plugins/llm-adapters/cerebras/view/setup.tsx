...
function mergeConfig(raw: unknown): CerebrasConnectionConfig {
  const stored = (raw ?? {}) as Partial<CerebrasConnectionConfig>;
  return {
    api_key: stored.api_key ?? "",
  };
}

export function CerebrasSetup({ connectionId, config, setConfig }: LlmProviderSetupProps) {
  const [current, setCurrent] = useState<CerebrasConnectionConfig>(() => mergeConfig(config));
  const [status, setStatus] = useState<"idle" | "saving" | "saved" | "error">("idle");

  const field = (key: keyof CerebrasConnectionConfig, value: string) => {
    ...
    <div key={connectionId} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: "var(--text-sm)", color: "var(--ink-3)" }}>
        ...
        <Input
          ...
          placeholder="csk-… — cloud.cerebras.ai"
