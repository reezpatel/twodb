...
function resolveEndpoint(config: Record<string, unknown>): { url: string; headers: Record<string, string> } {
  const apiKey = asText(config["api_key"]).trim();
  if (!apiKey) throw new Error("missing api_key in connection config");
  const baseUrl = asText(config["base_url"]).trim().replace(/\/+$/, "");
  return {
    url: `${baseUrl || DEFAULT_BASE_URL}/v1/messages`,
    headers: {
      "x-api-key": apiKey,
...
const AnthropicAdapter: TwodbLlmProviderAdapter = {
  providerId: "io.twodb.llm.anthropic",
...
const AnthropicServicePlugin = {
  init: async (_ctx: TwodbContext, app: FastifyInstance) => {
    app.invoke("llm.register", AnthropicAdapter);
  },
} satisfies ServicePlugin;

export default AnthropicServicePlugin;
