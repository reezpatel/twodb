...
const GCP_NAME_PATTERN = /^[a-z0-9][a-z0-9-]*$/;
...
function toRequestBody(request: TwodbCompletionRequest, options: VertexClientOptions): GeminiRequestBody {
...
function modelUrl(options: VertexClientOptions, model: string, stream: boolean): string {
  if (!GCP_NAME_PATTERN.test(options.project)) throw new Error(`invalid GCP project id: ${options.project}`);
  if (!GCP_NAME_PATTERN.test(options.location)) throw new Error(`invalid Vertex AI location: ${options.location}`);
  if (!MODEL_ID_PATTERN.test(model)) throw new Error(`invalid model id: ${model}`);
  const action = stream ? "streamGenerateContent?alt=sse" : "generateContent";
  return `https://${options.location}-aiplatform.googleapis.com/v1/projects/${options.project}/locations/${options.location}/publishers/google/models/${model}:${action}`;
}

async function postForResponse(endpoint: string, serviceAccountJson: string, body: GeminiRequestBody): Promise<Response> {
...
export async function completeGenerateContent(request: TwodbCompletionRequest, options: VertexClientOptions): Promise<TwodbCompletionResponse> {
  const endpoint = modelUrl(options, request.model, false);
  const response = await postForResponse(endpoint, options.serviceAccountJson, toRequestBody(request, options));
  const text = await response.text();
  if (!response.ok) throw new Error(`Vertex AI error ${response.status}: ${sanitizeError(text)}`);
  return toTwodbResponse(request, parsePayload(text));
}

export async function* streamGenerateContent(request: TwodbCompletionRequest, options: VertexClientOptions): AsyncGenerator<TwodbStreamEvent> {
  const endpoint = modelUrl(options, request.model, true);
  const response = await postForResponse(endpoint, options.serviceAccountJson, toRequestBody(request, options));
  if (!response.ok) {
    throw new Error(`Vertex AI error ${response.status}: ${sanitizeError(await response.text())}`);
  }
  if (!response.body) throw new Error("Vertex AI returned an empty response body");
