...
function mergeConfig(raw: unknown): GoogleConnectionConfig {
  const stored = (raw ?? {}) as Partial<GoogleConnectionConfig>;
  return { api_key: stored.api_key ?? "" };
}

export function GoogleSetup({ connectionId, config, setConfig }: LlmProviderSetupProps) {
  const [current, setCurrent] = useState<GoogleConnectionConfig>(() => mergeConfig(config));
  const [status, setStatus] = useState<"idle" | "saving" | "saved" | "error">("idle");

  const field = (key: keyof GoogleConnectionConfig, value: string) => {
  ...
  return (
    <div key={connectionId} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: "var(--text-sm)", color: "var(--ink-3)" }}>
        API key
        <Input type="password" defaultValue={current.api_key} placeholder="AIza…" onBlur={(event) => field("api_key", event.target.value.trim())} />
      </label>
