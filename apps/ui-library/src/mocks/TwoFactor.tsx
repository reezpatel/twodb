...
function makeCodes(seed: number) {
  ...
  return Array.from({ length: 8 }, (_, i) =>
    Array.from({ length: 8 }, (_, j) => alphabet[(seed * 31 + i * 17 + j * 7) % alphabet.length]).join(""),
  );
}
      ...
      <div className="mock-2fa">
        <div className="mock-2fa__panel">
          ...
          <p className="mock-2fa__desc">
            Your account now asks for a one-time code after your password. Keep these backup codes
        ...
        <p className="mock-2fa__hint">This is the state after a successful setup.</p>
      </div>
    );
  }
        ...
        <div className="mock-2fa__step">
          ...
          <div className="mock-2fa__qr-row">
            <div className="mock-2fa__qr-frame">
              <QRCode value={QR_URI} size={168} label="QR code linking twodb to your authenticator app" />
            </div>
            <div className="mock-2fa__qr-side">
              <span className="mock-2fa__side-title">Can't scan QR code?</span>
              <span className="mock-2fa__side-sub">Enter this secret instead:</span>
              <code className="mock-2fa__secret tw-tnum">ZPFI Y52G ITG7 WOWJ 44</code>
              <div>
