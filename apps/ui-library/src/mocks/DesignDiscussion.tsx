...
const ACTIVITY = [
  0.15, 0.3, 0.15, 0.55, 0.3, 0.7, 0.15, 0.4, 0.85, 0.3,
  0.55, 0.15, 0.7, 0.4, 0.15, 0.9, 0.55, 0.3, 0.15, 0.7,
];
...
/* renders text with @mentions lit */
...
function ReactionChip({ emoji, count }: { emoji: string; count: number }) {
  ...
  return (
    <button
      type="button"
      className={active ? "mock-dd__reaction mock-dd__reaction--active" : "mock-dd__reaction"}
...
function Post({ post, last }: { post: Post; last?: boolean }) {
  return (
    <article className={last ? "mock-dd__post mock-dd__post--last" : "mock-dd__post"}>
...
function Composer({ onSend }: { onSend: (text: string) => void }) {
  ...
  const suggestions = atMatch
    ? MEMBERS.filter((m) => m.name.toLowerCase().includes(atMatch[1].toLowerCase())).slice(0, 4)
    : [];
    ...
    <div className="mock-dd__composer">
      {suggestions.length ? (
        <div className="mock-dd__mentions">
          ...
          {suggestions.map((m) => (
            <button key={m.name} type="button" className="mock-dd__mention-row" onClick={() => insertMention(m.name)}>
...
export function DesignDiscussionMock() {
  ...
  function send(text: string) {
    setPosts((cur) => [
      ...cur,
      { id: `p${Date.now()}`, author: "You", when: "now", body: withMentions(text) },
    ]);
  }
      ...
      <aside className="mock-dd__side">
        ...
        <nav className="mock-dd__quick">
          <span className="mock-dd__quickitem">
            <Sparkles size={15} /> Assistant <Badge tone="rose" size="sm">New</Badge>
          </span>
          <span className="mock-dd__quickitem"><FileText size={15} /> Drafts</span>
          <span className="mock-dd__quickitem"><Bookmark size={15} /> Saved items</span>
          <span className="mock-dd__quickitem">
        ...
        <nav className="mock-dd__channels">
          {CHANNELS.map((c) => (
            <button
              key={c.id}
              type="button"
              className={c.id === channel ? "mock-dd__chan mock-dd__chan--active" : "mock-dd__chan"}
              style={{ paddingLeft: 8 + c.depth * 16 }}
              onClick={() => setChannel(c.id)}
            >
              {c.depth === 0 ? <span className="mock-dd__hash">#</span> : <span className="mock-dd__hash">↳</span>}
              {c.label}
      ...
      <section className="mock-dd__thread">
        <header className="mock-dd__thread-head">
          <span className="mock-dd__crumbs">
            <span className="mock-dd__hash">#</span> Website / v3.0 / <strong>UI-kit design</strong>
          </span>
      ...
      <aside className="mock-dd__info">
        ...
        {infoTab === "info" ? (
          <>
            <section className="mock-dd__infosec">
              ...
              <dl className="mock-dd__meta">
                <div><dt>Creator</dt><dd>Andrew M.</dd></div>
                <div><dt>Date of creation</dt><dd className="tw-tnum">28 May</dd></div>
                <div><dt>Status</dt><dd><Badge tone="go" size="sm">Active</Badge></dd></div>
                <div><dt>Tags</dt><dd className="tw-tnum">13</dd></div>
                <div><dt>Tasks</dt><dd className="tw-tnum">4</dd></div>
              </dl>
            </section>
            ...
            <section className="mock-dd__infosec">
              ...
              <div className="mock-dd__linked">
                <span><span className="mock-dd__hash">#</span> Front-end <b className="tw-tnum">4</b></span>
                <span><span className="mock-dd__hash">#</span> UI-kit design standards</span>
              </div>
            </section>
            ...
            <section className="mock-dd__infosec">
              <h4>Members <b className="tw-tnum">9</b></h4>
              <div className="mock-dd__members">
                {MEMBERS.map((m) => (
                  <div key={m.name} className="mock-dd__member">
                    ...
                    <Badge
                      size="sm"
                      tone={m.team === "Design" ? "go" : m.team === "Management" ? "warning" : "rose"}
                    >
                      {m.team}
                    </Badge>
                  </div>
          ...
          <div className="mock-dd__alt">
            <div className="mock-dd__linkcard">
              <span className="mock-dd__linkicon"><Pin size={14} /></span>
              <div className="mock-dd__linktext">
          ...
          <div className="mock-dd__alt">
            {["Lottie-animations.zip · 4.1 MB", "ui-kit-v3.fig · 12 MB"].map((f) => (
              <div key={f} className="mock-dd__linkcard">
                <span className="mock-dd__linkicon"><FileText size={14} /></span>
                <div className="mock-dd__linktext"><strong>{f.split(" · ")[0]}</strong><span>{f.split(" · ")[1]}</span></div>
              </div>
            ))}
          </div>
        ) : (
          <div className="mock-dd__alt">
            <div className="mock-dd__linkcard">
              <span className="mock-dd__linkicon"><Box size={14} /></span>
              <div className="mock-dd__linktext">
