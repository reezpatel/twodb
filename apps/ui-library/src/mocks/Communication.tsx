...
import { Avatar, Button, IconButton, Tabs, Textarea } from "@twodb/ui";
import {
	ChevronDown,
	ChevronUp,
	Lightbulb,
...
const WAVEFORM = [
	5, 9, 14, 20, 12, 8, 16, 22, 26, 18, 10, 7, 13, 21, 25, 28, 22, 14, 9, 15, 24,
	27, 19, 11, 6, 10, 17, 23, 26, 20, 12, 8, 14, 18, 22, 16, 10, 6, 12, 19, 24,
	21, 13, 8, 5, 9, 15, 11, 7, 4,
];

const TRANSCRIPT = [
	{
		who: "Tyler Bennett",
		text: "Hi, I need to move my appointment — something came up at work.",
	},
	{
		who: "Reception",
		text: "Of course. We have December 1st at 12:00 PM open, does that suit you?",
	},
	{
		who: "Tyler Bennett",
		text: "That's perfect. You'll send the confirmation over email?",
	},
	{
		who: "Reception",
		text: "Yes — it's on its way now, along with the preparation notes.",
	},
];
...
const HISTORY = [
	{
		...
		initials: "DR",
		tone: "neutral",
		text: "Hello. Unfortunately, none of these date…",
		time: "13:21",
	},
	{
		id: "h2",
		...
		initials: "WL",
		tone: "cobalt",
		text: "The cost of removing one wisdom tooth…",
		time: "12:37",
	},
	{
		id: "h3",
		...
		<div className="mock-cm">
			...
			<section className="mock-cm__thread">
						...
						<div className="mock-cm__bars" aria-hidden="true">
							{WAVEFORM.map((h, i) => (
								<i
									...
									style={{ height: h }}
									className={
										playing ? "mock-cm__bar mock-cm__bar--live" : "mock-cm__bar"
									}
								/>
							))}
						</div>
				...
				<div className="mock-cm__bubble-row">
					<div className="mock-cm__bubble">
						<p>
							Dear Mr. Tyler,
							<br />
							<br />I confirm the appointment reservation for December 1st, 2025
							at 12:00 PM.
						</p>
				...
				<div
					className={
						suggestionDone
							? "mock-cm__suggest mock-cm__suggest--done"
							: "mock-cm__suggest"
					}
				>
					...
					<IconButton
						...
						className={
							suggestionDone
								? "mock-cm__suggest-check mock-cm__suggest-check--done"
								: "mock-cm__suggest-check"
						}
						onClick={() => {
							if (!suggestionDone) {
					...
					{HISTORY.map((h) =>
						h.kind === "divider" ? (
							...
							<div key={h.id} className="mock-cm__hist-row">
								<span
									className={`mock-cm__hist-avatar mock-cm__hist-avatar--${h.tone}`}
								>
									{h.initials}
								</span>
