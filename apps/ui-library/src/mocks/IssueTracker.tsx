...
function ProgressBadge({ status }: { status: Progress }) {
	...
	return (
		<span className={`mock-issues__progress mock-issues__progress--${status}`}>
			<span className="mock-issues__progress-icon">{config[status].icon}</span>
			{config[status].label}
		</span>
	);
}
...
export function IssueTrackerMock() {
