...
const COMPANIES: Company[] = [
  { id: "payverge", name: "Payverge", path: "Payments Dig · Merchant", sector: "Home Services", score: 12 },
  { id: "paylocity", name: "Paylocity", path: "Payments Dig · Fintech", sector: "Finance", score: 23 },
  { id: "flowtap", name: "Flowtap", path: "Payments Dig · Merchant", sector: "Food & Beverages", score: 43 },
  { id: "adyn", name: "Adyn", path: "Payments Dig · Merchant", sector: "Finance", score: 61 },
  { id: "granitek", name: "Granitek", path: "Payments Dig · Merchant", sector: "Construction", score: -28 },
  { id: "vitavera", name: "Vitavera", path: "Payments Dig · Merchant", sector: "Healthcare", score: 32 },
  { id: "veloura", name: "Veloura", path: "Streetwear fashion · Merchant", sector: "Fashion", score: 78 },
  { id: "forgebuilt", name: "ForgeBuilt", path: "Payments Dig · Merchant", sector: "Construction", score: 8 },
];
...
function Delta({ value }: { value: string }) {
  ...
  return (
    <span className={up ? "mock-inv__delta mock-inv__delta--up" : "mock-inv__delta"}>{value}</span>
  );
}

function MetricCard({ label, value, ring }: { label: string; value: string; ring?: number }) {
  return (
    <div className="mock-inv__metric">
      {ring !== undefined ? <ScoreRing value={ring} size={40} stroke={4} /> : null}
      <div>
  ...
  return (
    <div className="mock-inv__detail">
      ...
      <div className="mock-inv__revenue">
        ...
        <div className="mock-inv__bar" role="img" aria-label="Revenue split: product 18%, services 34%, licensing 28%, other 20%">
          {REVENUE_SEGMENTS.map((s) => (
            <span key={s.label} style={{ background: s.tone, width: `${s.width}%` }} />
          ))}
        </div>
        ...
        <div className="mock-inv__details-grid">
          ...
          <div className="mock-inv__detail-item mock-inv__detail-item--wide">
            ...
            <span className="mock-inv__detail-value">
              Paylocity was established in 2008, and is primarily known for cloud-based payroll
              and human capital management (HCM) software solutions for businesses.
            </span>
          </div>
          <div className="mock-inv__detail-item mock-inv__detail-item--wide">
        ...
        <div className="mock-inv__companies">
          {COMPANIES.map((c) => (
            <button
              ...
              type="button"
              className={
                c.id === companyId ? "mock-inv__company mock-inv__company--active" : "mock-inv__company"
              }
            ...
            >
              ...
              <span className="mock-inv__company-foot">
                <Badge tone="neutral" size="sm">{c.sector}</Badge>
      ...
      <div className="mock-inv__main">
        <header className="mock-inv__head">
          ...
          <div className="mock-inv__head-text">
            <span className="mock-inv__company-path">{company.path} · Payment solutions</span>
            <h3>{company.name}</h3>
          </div>
        ...
        <div className="mock-inv__metrics">
          <MetricCard label="Overall score" value={String(company.score)} ring={company.score} />
          <MetricCard label="Revenue goal attainment" value="87%" ring={87} />
          <MetricCard label="Forecast this month" value="1.4M" ring={64} />
          <button type="button" className="mock-inv__metric mock-inv__metric--add">
            + Add segment
          </button>
        </div>
        ...
        {tab === "overview" ? (
          <div className="mock-inv__concentration">
            <span className="mock-inv__kpi-title">Customer concentration (% revenue)</span>
                ...
                <table className="tw-table">
                  <thead className="tw-thead">
                    <tr className="tw-tr">
                      ...
                      {YEARS.map((y) => (
                        <th key={y} className="tw-th tw-th--right tw-tnum">{y}</th>
                      ))}
                    </tr>
                  </thead>
                    ...
                    {CUSTOMERS.map((cust) => {
                      ...
                      return (
                        <>
                          <tr
                            key={cust.name}
                            className={open ? "tw-tr mock-inv__row mock-inv__row--open" : "tw-tr mock-inv__row"}
                          ...
                          >
                            ...
                            {cust.years.map((y, i) => (
                              <td key={i} className="tw-td tw-td--right">
                                <span className="tw-tnum">{y.pct}</span> <Delta value={y.delta} />
                              </td>
                            ))}
                          </tr>
                          {open ? (
          ...
          <div className="mock-inv__empty">
            <p>The {tab} view fills in once a real data source is connected — this mock ships Overview.</p>
          </div>
        )}
      </div>
    </div>
