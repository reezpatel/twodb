	...
	return (
		<>
			<SettingGroup
				...
				description="How you appear to other people in the workspace."
			>
				<SettingRow
					label="Profile photo"
					description="A clear photo helps your team find you."
				>
				...
				<SettingRow label="Full name">
					<div className="mock-set__field">
						<Input defaultValue="Dr. Asha Verma" aria-label="Full name" />
					</div>
				</SettingRow>
				<SettingRow label="Headline" description="One line under your name.">
					<div className="mock-set__field">
						<Input
							defaultValue="General physician, Verma Clinic"
							aria-label="Headline"
						/>
					</div>
				</SettingRow>
				<SettingRow label="Bio">
					<div className="mock-set__field">
						<Textarea
							defaultValue={
								"Family clinic in Nashik since 2011.\nDiabetes care, vaccinations, home visits on Thursdays."
							}
							aria-label="Bio"
						/>
					</div>
				...
				<SettingRow label="Profile link">
					<div className="mock-set__linkrow">
						...
						<div className="mock-set__field">
							<Input defaultValue="dr-asha-verma" aria-label="Profile link" />
						</div>
			...
			<SettingGroup label="Contact">
				<SettingRow label="Email" control={<Badge tone="go">Verified</Badge>}>
					<div className="mock-set__field">
						<Input
							defaultValue="asha@vermaclinic.in"
							type="email"
							aria-label="Email"
						/>
					</div>
				...
				>
					<div className="mock-set__field">
						<Input defaultValue="+91 98220 41xx" aria-label="Phone" />
					</div>
				</SettingRow>
				<SettingRow
					label="Language"
...
function AccountSection() {
	...
	const [pwState, setPwState] = useState<null | "ok" | "mismatch" | "short">(
		null,
	);
	...
	return (
		<>
			<SettingGroup
				...
				description="At least 8 characters. Sessions elsewhere stay signed in."
			>
				...
				<SettingRow label="New password">
					<div className="mock-set__field">
						<PasswordInput
							...
							error={
								pwState === "short" ? "Use at least 8 characters" : undefined
							}
						/>
					</div>
				</SettingRow>
				<SettingRow label="Confirm new password">
					<div className="mock-set__field">
						<PasswordInput
							...
							error={
								pwState === "mismatch" ? "Passwords don't match" : undefined
							}
						/>
					</div>
				</SettingRow>
				<SettingRow label="">
					<div className="mock-set__savebar" style={{ padding: 0 }}>
						...
						{pwState === "ok" ? (
							<Badge tone="go">
								<Check size={11} aria-hidden="true" /> Password updated
							</Badge>
						) : null}
					</div>
				</SettingRow>
			...
			<SettingGroup
				...
				description="A second proof when you sign in on a new device."
			>
				<SettingRow
					label="Two-factor authentication"
					description={
						twoFa
							? "On — codes by authenticator app"
							: "Off — your account is easier to break into"
					}
					control={
						<Switch
							checked={twoFa}
			...
			>
				{sessions.map((s) => (
					<SettingRow
						...
						label={s.name}
						description={
							s.current ? `${s.where} · this device` : `${s.where} · ${s.when}`
						}
						control={
							s.current ? (
				...
				<SettingRow
					...
					control={
						<Button size="sm" variant="danger">
							<LogOut size={14} aria-hidden="true" /> Log out all
						</Button>
					}
				/>
			</SettingGroup>
			...
			>
				Your notes, automations, and apps are erased after 14 days. Download an
				archive first if you need one — this cannot be undone.
			</Dialog>
		</>
	);
}
...
function PrivacySection() {
	...
	const [archive, setArchive] = useState<"idle" | "preparing" | "ready">(
		"idle",
	);
	...
	return (
		<>
			<SettingGroup
				...
				description="Applies to search, your profile page, and the directory."
			>
				<SettingRow
				...
				<SettingRow
					...
					control={
						<Switch
							checked={switches.activity}
							onChange={() => flip("activity")}
				...
				<SettingRow
					...
					control={
						<Switch
							checked={switches.indexing}
							onChange={() => flip("indexing")}
			...
			<SettingGroup label="Messaging">
				...
				<SettingRow
					...
					control={
						<Switch
							checked={switches.mentions}
							onChange={() => flip("mentions")}
							aria-label="Allow mentions"
						/>
					}
				/>
			</SettingGroup>
					...
					control={
						archive === "idle" ? (
							<Button
								...
								variant="secondary"
								onClick={() => {
									setArchive("preparing");
									setTimeout(() => setArchive("ready"), 2000);
								}}
							>
								Request archive
							</Button>
			...
			<SettingGroup
				...
				description={`${blocked.length} blocked — they can't find or message you.`}
			>
				{blocked.map((b) => (
					<SettingRow
						key={b.id}
						...
						control={
							<Button
								...
								variant="ghost"
								onClick={() =>
									setBlocked((c) => c.filter((x) => x.id !== b.id))
								}
							>
								Unblock
							</Button>
...
const NOTIF_ROWS = [
	{
		...
		desc: "Someone @mentions you or replies to you.",
	},
	{
		id: "messages",
		label: "Direct messages",
		desc: "New messages in one-to-one chats.",
	},
	{
		id: "invoices",
		label: "Invoice updates",
		desc: "Paid, overdue, or failed payments.",
	},
	{
		id: "brief",
		label: "Morning brief",
		desc: "Your daily brief is ready to read.",
	},
	{
		id: "automations",
		label: "Automation results",
		desc: "Runs that finished or need attention.",
	},
	{
		id: "product",
		label: "Product news",
		desc: "New features, at most once a month.",
	},
] as const;
			...
			>
				<div className="mock-set__matrix">
					<div className="mock-set__matrix-head">
						<span />
						{CHANNELS.map((c) => (
							<span key={c.id} className="tw-cue">
								{c.label}
							</span>
						))}
					</div>
				...
				{quiet ? (
					<SettingRow label="From">
						<div className="mock-set__quiet">
							<TimePicker
								defaultValue={new Date(2026, 0, 1, 22, 0)}
								aria-label="Quiet from"
							/>
							<span className="tw-srow__desc">to</span>
							<TimePicker
								defaultValue={new Date(2026, 0, 1, 7, 0)}
...
function AppearanceSection() {
	...
	return (
		<>
			<SettingGroup
				label="Theme"
				description="This changes the real showcase phase — try Night."
			>
			...
			<SettingGroup label="Density">
				...
				<SettingRow
					label="Preview"
					description="The frame below follows your density choice."
				>
