...
const CATS = [
	{ id: "standup", label: "Standups", color: "#c2285a", tint: "var(--rose-soft-bg)" },
	{ id: "patients", label: "Appointments", color: "#0f9d8f", tint: "rgb(15 157 143 / 0.10)" },
	{ id: "meetings", label: "Meetings", color: "#3a55ff", tint: "var(--accent-soft-bg)" },
	{ id: "breaks", label: "Breaks", color: "#d9a03f", tint: "rgb(217 160 63 / 0.14)" },
	{ id: "admin", label: "Admin", color: "#7c3aed", tint: "rgb(124 58 237 / 0.10)" },
];
...
const EVENTS: CalEvent[] = [
	...
	{ title: "Lunch break", day: 3, start: 12, end: 13, cat: "breaks", people: [] },
	{ title: "Ward 4 rounds", day: 4, start: 8, end: 9, cat: "patients", people: ["Asha Verma", "Ravi Kumar"] },
	{ title: "Pharma order review", day: 4, start: 11, end: 12, cat: "admin", people: ["Dev Patel", "Meera Iyer"] },
	{ title: "Vaccination camp", day: 5, start: 10, end: 12, cat: "patients", people: ["Meera Iyer", "Ravi Kumar"], more: 8 },
];
...
export function WeeklyCalMock() {
	const [anchor, setAnchor] = useState(() => new Date(2024, 2, 7)); /* Thu 7 Mar 2024 */
	...
	const visible = EVENTS.filter((e) => enabled[e.cat] && (view !== "day" || e.day === (anchor.getDay() + 6) % 7));
			...
			<main className="mock-wc__main">
				<header className="mock-wc__head">
					<div>
						...
						<Badge tone="neutral">{visible.length} events this {view === "day" ? "day" : "week"}</Badge>
					</div>
					<div className="mock-wc__headctl">
						...
						/>
						<Button variant="secondary" size="sm" onClick={() => shift(view === "day" ? -1 : -7)} aria-label="Previous">
							<ChevronLeft aria-hidden="true" />
						</Button>
						<Button variant="secondary" size="sm" onClick={() => setAnchor(new Date(2024, 2, 7))}>
							Today
						</Button>
						<Button variant="secondary" size="sm" onClick={() => shift(view === "day" ? 1 : 7)} aria-label="Next">
				...
				{view === "month" ? (
					<div className="mock-wc__monthview">
						<Calendar
							...
							onMonthChange={setAnchor}
						/>
						<p className="mock-wc__monthhint">Pick a day to jump into its week.</p>
					</div>
				) : (
					<div className="mock-wc__grid" style={{ gridTemplateColumns: `52px repeat(${cols}, minmax(0, 1fr))` }}>
						{/* day header row */}
						<span className="mock-wc__tz">UTC +5:30</span>
						{(view === "day" ? [anchor] : days).map((d, i) => (
							<div
								className={d.toDateString() === anchor.toDateString() ? "mock-wc__dhead is-today" : "mock-wc__dhead"}
								key={i}
							>
										...
										return (
											...
											>
												...
												<span className="mock-wc__etime">
													<Clock aria-hidden="true" /> {fmtHour(e.start)} – {fmtHour(e.end)}
												</span>
												{e.people.length ? (
