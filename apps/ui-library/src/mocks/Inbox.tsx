...
const THREADS: Thread[] = [
  ...
  { id: "liam", from: "Liam", count: 10, time: "09:50", snippet: "It's still a bit rough, but I'm open to…", unread: true },
...
const LABELS = [
  { id: "marketing", label: "Marketing", count: 35, tone: "var(--twdb-cobalt)" },
  { id: "finance", label: "Finance", count: 20, tone: "var(--twdb-rose)" },
  { id: "operation", label: "Operation", count: 14, tone: "var(--twdb-rose-light)" },
];
...
function ThreadRow({
  ...
  return (
    <button
      type="button"
      className={active ? "mock-in__thread mock-in__thread--active" : "mock-in__thread"}
      onClick={onSelect}
    >
      ...
      {thread.unread ? <i className="mock-in__unread" aria-label="Unread" /> : null}
    </button>
  );
}
    ...
    <article className="mock-in__msg">
      <header className="mock-in__msg-head">
        ...
        <Menu
          ...
          trigger={<IconButton size="sm" label="More actions" icon={<CornerUpRight />} />}
        >
      ...
      <aside className="mock-in__folders">
        ...
        <NavSection
          items={FOLDERS.map((f) => ({
            ...
            label: f.label,
            icon: f.icon,
            badge: f.count ? <span className="tw-tnum mock-in__count">{f.count}</span> : undefined,
          }))}
          value={folder}
        ...
        <div className="mock-in__threads">
          {pinned.length ? (
            <>
              ...
              {pinned.map((t) => (
                <ThreadRow key={t.id} thread={t} active={selected === t.id} onSelect={() => setSelected(t.id)} />
              ))}
            </>
          ) : null}
          {rest.length ? (
      ...
      <section className="mock-in__read">
        {selected === "sitemap" ? (
          <>
            <div className="mock-in__toolbar">
              ...
              <span className="mock-in__pager tw-tnum">
                1 from 340
                <IconButton size="sm" variant="secondary" label="Newer" icon={<ChevronLeft />} />
                <IconButton size="sm" variant="secondary" label="Older" icon={<ChevronRight />} />
              </span>
              <IconButton label="Close conversation" icon={<ChevronRight />} onClick={() => setSelected(null)} />
            </div>

            <div className="mock-in__scroll">
              <span className="mock-in__date tw-tnum">June 24, 2026 · 10:15 AM</span>
              ...
              <div className="mock-in__summary">
                ...
                <p>
                  The navigation layout has been revamped, and a new section for quick access to
                  frequently used features has been added. Margins have been adjusted to meet
                  updated design standards. Feedback is requested before moving forward.
                </p>
              </div>
          ...
          <div className="mock-in__empty">
            <p>
              <strong>{THREADS.find((t) => t.id === selected)?.from}</strong> — this conversation's
              messages come with the mail connection. The sitemap thread is fully mocked.
            </p>
            <Button size="sm" variant="secondary" onClick={() => setSelected("sitemap")}>
