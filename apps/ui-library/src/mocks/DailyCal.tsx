...
const CATS = [
	{ id: "clinic", label: "Clinic", color: "#3a55ff", tint: "var(--accent-soft-bg)" },
	{ id: "patients", label: "Patients", color: "#0f9d8f", tint: "rgb(15 157 143 / 0.10)" },
	{ id: "admin", label: "Admin", color: "#7c3aed", tint: "rgb(124 58 237 / 0.10)" },
	{ id: "personal", label: "Personal", color: "#d9a03f", tint: "rgb(217 160 63 / 0.14)" },
];
...
export function DailyCalMock() {
	...
	const visible = useMemo(() => day.events.filter((e) => enabled[e.cat]), [day, enabled]);
		...
		<div className="mock-dc">
			...
			<aside className="mock-dc__rail">
				<div className="mock-dc__profile">
					...
					<Badge tone="rose" size="sm">Clinic admin</Badge>
				</div>
				<nav className="mock-dc__nav" aria-label="Menu">
					<span className="mock-dc__navlabel">Menu</span>
					{NAV.map((n) => (
						<span className={n.active ? "mock-dc__navitem is-active" : "mock-dc__navitem"} key={n.label}>
				...
				<div className="mock-dc__cats">
					...
					<div className="mock-dc__chips">
						{CATS.map((c) => (
							<button
								className={enabled[c.id] ? "mock-dc__chip" : "mock-dc__chip is-off"}
			...
			<main className="mock-dc__day">
				<div className="mock-dc__timeline">
					...
					<div className="mock-dc__lane">
						...
						{/* now-line */}
						<span className="mock-dc__now" style={{ top: (NOW - DAY_START) * HOUR_H }}>
							<em>{fmt(NOW)}</em>
						</span>
						{visible.map((e, i) => {
							...
							return (
								...
								>
									...
									{e.note ? <span className="mock-dc__enote">{e.note}</span> : null}
									<span className="mock-dc__emeta">
										{fmt(e.start)} – {fmt(e.end)} · Dr. Asha
									</span>
									{e.star ? <Star aria-hidden="true" className="mock-dc__estar" /> : null}
									{done.has(id) ? <span className="mock-dc__edone">Done</span> : null}
								</div>
							);
						})}
					</div>
			...
			<aside className="mock-dc__side">
				<header className="mock-dc__sidehead">
					<IconButton icon={<ChevronLeft />} label="Previous day" variant="ghost" size="sm" onClick={() => setDayIdx((i) => (i + DAYS.length - 1) % DAYS.length)} />
					<h3>
						{day.label} · <span>{day.date.getDate()} March</span>
					</h3>
					<IconButton icon={<ChevronRight />} label="Next day" variant="ghost" size="sm" onClick={() => setDayIdx((i) => (i + 1) % DAYS.length)} />
				</header>
				...
				<div className="mock-dc__mini">
					<Calendar
						...
						onSelect={(d) => {
							if (!d) return;
							const i = DAYS.findIndex((x) => x.date.toDateString() === d.toDateString());
							if (i >= 0) setDayIdx(i);
						}}
						month={day.date}
						onMonthChange={() => {}}
				...
				<div className="mock-dc__agenda">
					{day.events.map((e, ei) => {
						...
						return (
							<button
								className={
									"mock-dc__agitem" + (isDone ? " is-done" : "") + (off ? " is-off" : "")
								}
								key={ei}
							...
							>
								<span className="mock-dc__agcheck">
									{isDone ? <CheckCircle2 aria-hidden="true" /> : <Circle aria-hidden="true" />}
								</span>
					...
					{!day.events.length ? <p className="mock-dc__agempty">Nothing scheduled.</p> : null}
				</div>
