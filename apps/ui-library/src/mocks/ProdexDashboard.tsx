...
const ACTIVITIES = [
	{
		title: "Order #2048",
		detail: "John Doe · 12 Jan 25",
		tag: "New Order",
		tone: "go" as const,
	},
	{
		title: "Low Stock Alert",
		detail: "MacBook Air M2 · 10 Jan 25",
		tag: "Low Stock",
		tone: "danger" as const,
	},
	{
		title: 'Promo code "SUMMER20"',
		detail: "Applied 52 times · 8 Jan 25",
		tag: "Campaign",
		tone: "rose" as const,
	},
	{
		title: "System Update",
		detail: "Version 1.21 · 2 Jan 25",
		tag: "System",
	...
	return (
		<section className="mock-prodex__panel mock-prodex__revenue">
			<header className="mock-prodex__panel-head">
				<div>
					<span className="mock-prodex__mini-icon">
						<WalletCards />
					</span>
					...
					{REVENUE.map((item) => (
						<div className="mock-prodex__barcol" key={item.month}>
							<div
								className={
									item.active
										? "mock-prodex__bar is-active"
										: "mock-prodex__bar"
								}
							>
								<span
									className="mock-prodex__bar-one"
									style={{ height: `${item.one}px` }}
								/>
								<span
									className="mock-prodex__bar-rec"
									style={{ height: `${item.recurring}px` }}
								/>
								{item.active ? (
									<span className="mock-prodex__tip">
										<b>One-Time Revenue</b>$6,000
										<br />
										<b>Recurring Revenue</b>$25,000
									</span>
								) : null}
							</div>
...
function CategoryRing() {
	return (
		<section className="mock-prodex__panel mock-prodex__category">
			...
			<div
				className="mock-prodex__donut"
				aria-label="Top categories donut chart"
			>
				...
				<div>
					...
					<strong className="tw-tnum">$125,000</strong>
				</div>
			</div>
			<div className="mock-prodex__cat-list">
				{[
					...
					<div key={name}>
						<span className={`dot dot-${tone}`} /> <b>{name}</b>
						<em>{amount}</em>
						<strong>{pct}</strong>
					</div>
				))}
	...
	return (
		<div className="mock-prodex" aria-label="Prodex dashboard mock">
			<aside className="mock-prodex__side">
				<div className="mock-prodex__brand">
					<span>
						<Grid2X2 />
					</span>
					<strong>Prodex</strong>
					<PanelLeft />
				</div>
				<div className="mock-prodex__workspace">
					<span>UX</span>
					<b>Uxerflow</b>
					<ChevronDown />
				</div>
				<nav aria-label="Prodex navigation">
					...
					{NAV.map((item) => (
						...
						>
							{item.icon}
							<span>{item.label}</span>
							{item.open ? <ChevronUp /> : null}
						</button>
					))}
					<div className="mock-prodex__sub">
				...
				<div className="mock-prodex__dark">
					<Moon /> <span>Dark More</span>
					<i />
				</div>
				<div className="mock-prodex__upgrade">
					<Crown />
					<strong>
						Upgrade to <Badge tone="go">Premium</Badge>
					</strong>
					<p>
						Your Premium Account will expire in <b>18 days.</b>
					</p>
			...
			<main className="mock-prodex__main">
				<header className="mock-prodex__top">
					<h2>Dashboard</h2>
					<div className="mock-prodex__team">
						<Avatar name="Nia Cruz" size="sm" />
					...
					<button className="mock-prodex__bell">
						<Bell />
						<sup>24</sup>
					</button>
					<label className="mock-prodex__search">
						<Search />
						<input placeholder="Search anything" />
						<kbd>⌘ K</kbd>
					</label>
					<div className="mock-prodex__profile">
						<Avatar name="Asha Verma" size="sm" />
						<ChevronDown />
					</div>
				</header>

				<div className="mock-prodex__stats">
					{STATS.map((item) => (
				...
				<div className="mock-prodex__grid">
					...
					<section className="mock-prodex__panel mock-prodex__activity">
						...
						{ACTIVITIES.map((item) => (
							<div className="mock-prodex__activity-row" key={item.title}>
								<span
									className={`mock-prodex__activity-icon tone-${item.tone}`}
								>
									{item.tone === "danger" ? <Package /> : <Bell />}
								</span>
								<div>
					...
					<section className="mock-prodex__panel mock-prodex__products">
						<header className="mock-prodex__panel-head">
							...
							<div className="mock-prodex__tools">
								<Button size="sm" variant="secondary">
						...
						<table>
							...
							<tbody>
								{PRODUCTS.map((item) => (
									<tr key={item.product}>
										<td>
											<span className="mock-prodex__thumb" />
											{item.product}
										</td>
										<td>{item.stock}</td>
										<td>{item.price}</td>
