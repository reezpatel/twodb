...
export const envSchema = {
	...
	properties: {
		...
		TWO_DB_WORK_DIR: {
			...
			default: "../../../.work",
			description:
				"Directory holding the api-owned sqlite database (db-v1.sqlite), resolved from apps/api/src; created if missing",
		},
		...
		TWODB_ADMIN_ORIGIN: {
			...
			description:
				"Expected WebAuthn origin for admin passkey ceremonies (the web app origin)",
		},
		...
		TWODB_AGENT_ENCRYPTION_KEY: {
			...
			description:
				"Key material (hashed to 32 bytes) for AES-256-GCM encryption of agent credentials at rest. MUST be overridden outside dev.",
		},
		TWODB_AGENT_USAGE_INTERVAL_MS: {
			type: "number",
			default: 300_000,
			description:
				"Cron tick for agent usage collection. Providers are only refetched once their own TTL elapsed, so this can tick often.",
		},
		...
		TWODB_STORAGE_ENCRYPTION_KEY: {
			...
			description:
				"Key material (hashed to 32 bytes) for AES-256-GCM encryption of storage-location credentials at rest. MUST be overridden outside dev.",
		},
