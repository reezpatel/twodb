...
import { fileTreeNodeStyles, fileTreeStyles } from "./file-tree.style";
	...
	return (
		<div className="tw-ftree__node">
			...
			<button
				...
				className={
					isSelected ? "tw-ftree__row tw-ftree__row--selected" : "tw-ftree__row"
				}
				onClick={() => {
					onSelect?.(node.id);
			...
			>
				<span className="tw-ftree__icon">
					{node.icon ??
						(kids.length ? isOpen ? <FolderOpen /> : <Folder /> : <File />)}
				</span>
				<span className="tw-ftree__label">{node.label}</span>
				{node.count != null ? (
					<b className="tw-ftree__count tw-tnum">{node.count}</b>
				) : null}
				{kids.length ? (
					<span
						className={
							isOpen ? "tw-ftree__chev tw-ftree__chev--open" : "tw-ftree__chev"
						}
					>
