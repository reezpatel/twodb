	...
	return (
		<div className="tw-chat__header">
			<div className="tw-chat__title-block">
				...
				{subtitle ? (
					<span className="tw-chat__subtitle">{subtitle}</span>
				) : null}
			</div>
			{members ? (
				<div
					className="tw-chat__members"
				...
				>
					...
					{members.length > 4 ? (
						<span className="tw-chat__members-more tw-tnum">
							+{members.length - 4}
						</span>
					) : null}
				</div>
 ...
 * Renders above the message content with a left accent bar,
 * the original author's name + time, and a snippet of text.
 */
export interface ChatReplyTo {
	/** Stable id of the message being replied to; passed to onReplyClick. */
	id?: string;
	author: string;
	time?: string;
	/** Short snippet of the original message (1–2 lines, will be truncated). */
	text: string;
}

export interface MessageGroupProps {
	/** DOM id — useful as a scroll target for reply-to jumps. */
	id?: string;
	author: string;
	avatarSrc?: string;
...
export function ChatMessage({
	...
	return (
		<div className="tw-msg">
			...
			<div className="tw-msg__content">
				{replyTo ? (
					<button
						type="button"
						className="tw-msg__reply"
						aria-label={`Reply to ${replyTo.author}${replyTo.time ? `, ${replyTo.time}` : ""}`}
						onClick={() => onReplyClick?.(replyTo.id)}
					>
						<span className="tw-msg__reply-head">
							<span className="tw-msg__reply-author">{replyTo.author}</span>
							{replyTo.time ? (
								<span className="tw-msg__reply-time tw-tnum">
									{replyTo.time}
								</span>
							) : null}
						</span>
						<span className="tw-msg__reply-text">{replyTo.text}</span>
					</button>
				) : null}
				{children}
			</div>
		...
		<div className="tw-msg__gallery" data-count={Math.min(images.length, 4)}>
			{images.slice(0, 4).map((img, i) => (
				<div key={i} className="tw-msg__gallery-item">
					...
					{i === 3 && images.length > 4 ? (
						<span className="tw-msg__gallery-more tw-tnum">
							+{images.length - 4}
						</span>
					) : null}
				</div>
...
const WAVE = [
	0.35, 0.6, 0.9, 0.5, 0.75, 1, 0.65, 0.4, 0.8, 0.55, 0.95, 0.45, 0.7, 0.3, 0.6,
	0.85, 0.5, 0.7, 0.4, 0.62, 0.42, 0.78, 0.55, 0.34,
];
...
export function AudioMessage({ duration }: { duration: string }) {
	...
	return (
		<div className="tw-msg__audio">
			...
			<span
				className={
					playing ? "tw-msg__wave tw-msg__wave--playing" : "tw-msg__wave"
				}
				aria-hidden="true"
			>
				{WAVE.map((h, i) => (
