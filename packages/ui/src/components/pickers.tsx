  ...
  const field = (
    <div className="tw-picker" ref={anchorRef}>
      ...
      >
        ...
        <span className={display ? "tw-picker__value" : "tw-picker__placeholder"}>
          {display || placeholder}
        </span>
      </button>
...
function TimeFields({ value, onChange }: { value: Date; onChange: (d: Date) => void }) {
...
export function DateTimePicker({
  ...
  return (
    <FieldShell
      ...
      hint={hint}
      error={error}
      display={current ? `${DATE_FMT.format(current)}, ${TIME_FMT.format(current)}` : undefined}
      placeholder={placeholder}
      open={open}
      onToggle={() => !disabled && setOpen((o) => !o)}
      onClose={() => setOpen(false)}
    >
      ...
      <div className="tw-picker__time">
        <TimeFields value={current ?? new Date(new Date().setHours(9, 0, 0, 0))} onChange={commit} />
        <Button size="sm" variant="ghost" className="tw-picker__done" onClick={() => setOpen(false)}>
          Done
        </Button>
      </div>
    </FieldShell>
