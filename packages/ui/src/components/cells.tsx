...
export function CellView({
  ...
  switch (type) {
    ...
    case "currency":
      return <span className="tw-tnum">₹{Number(value ?? 0).toLocaleString("en-IN")}</span>;
    ...
    case "stars": {
      ...
      return (
        <span className="tw-cell-stars" aria-label={`${n} out of 5`}>
          {[1, 2, 3, 4, 5].map((i) => (
            <Star key={i} aria-hidden="true" className={i <= n ? "is-on" : undefined} />
          ))}
        </span>
      );
    }
...
export function CellEditor({ type, value, options, onCommit, onClose }: EditorProps) {
  ...
  if (type === "text" || type === "url" || type === "number" || type === "currency" || type === "file") {
    return (
      <input
  ...
  return (
    <span ref={anchorRef}>
      ...
      {createPortal(
        <div className="tw-cellpop" ref={popupRef} role="listbox" aria-multiselectable="true">
          {(options ?? []).map((opt) => {
            ...
            return (
              <button
                ...
                role="option"
                aria-selected={on}
                className={on ? "tw-cellpop__item tw-cellpop__item--selected" : "tw-cellpop__item"}
                onClick={() =>
                  onCommit(on ? selected.filter((v) => v !== opt.value) : [...selected, opt.value])
                }
              >
                <Badge size="sm" tone={opt.tone ?? "neutral"}>
                  {opt.label}
          ...
          <div className="tw-cellpop__foot">
            <button type="button" className="tw-cellpop__done" onClick={onClose}>
              Done
            </button>
          </div>
        </div>,
